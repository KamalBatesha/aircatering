...
function AccordionItem({ idPrefix, index, isOpen, onToggle, children, noAutoScroll }) {
  const itemRef = useRef(null);

  useEffect(() => {
    if (!isOpen) return;
    if (noAutoScroll) return;   // skip when opened programmatically (e.g. search)
    ...
    <div ref={itemRef}>
      <h2 id={`${idPrefix}-heading-${index}`}>
        <button
          ...
          onClick={onToggle}
          className="flex w-full justify-between bg-[#F5F5F5] px-5 py-2 rounded-xl"
        >
          {children.header}
          <svg
            data-accordion-icon
