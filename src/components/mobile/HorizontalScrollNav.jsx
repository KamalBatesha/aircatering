...
const HorizontalScrollNav = React.forwardRef(function HorizontalScrollNav({ data, activeIndex = 0, onSelect = () => { } }, ref) {
  ...
  const items = data?.map(grandGroup => ({
    title: lang == "EN" ? grandGroup?.FoodMenuGrandGroupName : grandGroup?.FoodMenuGrandGroupNameAr || grandGroup?.FoodMenuGrandGroupName,
  ...
  const updateUnderline = () => {
    ...
    // The underline is absolute inside a relative container that is the scrollable content.
    // So we just need the item's position relative to that inner container.
    const container = nav.querySelector('.inline-flex');
    if (!container) return;
    ...
    <div className="sticky top-0 z-20 w-full bg-white shadow-sm">
      <div className="max-w-4xl mx-auto relative border-b border-gray-200">
        <div className="flex items-center">
          ...
          >
            <div className="inline-flex items-center relative pb-3">
              {items.map((label, i) => (
                <button
                  key={`${label?.title}-${i}`}
                  ref={(el) => (itemRefs.current[i] = el)}
                  onClick={() => onSelect(i)}
                  className={`px-4 py-1 mr-3 text-sm font-medium transition-colors duration-150 ${activeIndex === i ? "text-black" : "text-gray-600"
                    }`}
                >
                  {label?.title}
                </button>
              ...
              <span
                ref={underlineRef}
                className="absolute bottom-px left-0 h-[3px] bg-primary transition-all duration-200 ease-out"
        ...
        <div className="flex flex-col overflow-y-auto  flex-1 py-2 px-4">
          {items.map((label, i) => (
            ...
            >
              ...
              <p className="text-gray">{lang == "EN" ? label?.count : toArabicNumbers(label?.count)}</p>
            </div>
          ))}
        </div>
      </div>
