...
function Menu({ scrollToItemId, orderDetails }) {
  ...
  const SUBGROUP_ITEMS_URL = (grandGroupId, subgroupId, stationId) => {
    const params = new URLSearchParams();
  ...
  const { data: groupsData, isLoading: isGroupsLoading } = useQuery({
    queryKey: ["menu-groups", selectedMenuHeaderId],
    queryFn: async () => {
  ...
  const { data: subgroupItems, isLoading: isItemsLoading } = useQuery({
    queryKey: ["subgroup-items", selectedSubgroup?.subGroupID, selectedStation?.stationId, selectedMainGroupID, selectedMenuHeaderId],
    queryFn: async () => {
    ...
    enabled: !!selectedSubgroup && !!selectedMainGroupID && !debouncedSearch.trim(),
  });
  ...
  const { data: searchResults, isLoading: isSearchLoading } = useQuery({
    queryKey: ["menu-search", debouncedSearch, selectedMenuHeaderId],
    queryFn: async () => {
  ...
  }, [groupsData, selectedMainGroupID]);
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
          <div className="flex items-center">
            {user ? (
              ...
              <div
                className="flex items-center text-gray-400 bg-gray-50 p-2 rounded-full gap-1.5"
          ...
          <button
            className="w-8 h-8 sm:w-9 sm:h-9 flex cursor-pointer items-center justify-center bg-primary text-white rounded-full hover:bg-[#b08848] active:scale-95 transition-all shadow-sm shadow-primary/30 shrink-0"
            onClick={(e) => {
              e.stopPropagation();
  ...
  return (
    <div dir={lang === 'AR' ? 'rtl' : 'ltr'} className="w-full flex font-sans max-w-[1400px] mx-auto">
      ...
      <div className="lg:w-1/4 hidden lg:block pe-6 relative">
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-5 flex flex-col sticky top-3 z-40">
          ...
          <div className="flex flex-col gap-1.5">
            {isGroupsLoading ? <BiLoaderAlt className="animate-spin mx-auto my-10 text-primary" size={30} /> : sidebarGroups.map((mainGroup) => {
                ...
                >
                  ...
                  >
                    <div className="flex justify-between w-full">
                      ...
                      <span className="truncate">{mainGroup?.mainGroupName}
                      </span>
                      <span
                        className={`shrink-0 text-[11px] font-semibold px-1.5 py-0.5 rounded-full ${isActive
                  ...
                  {hoveredGroupId === mainGroup?.mainGroupID && (
                    <div className="absolute left-[92%] -top-2 ml-2 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 p-4 transition-all duration-250">
                      <p className="text-[10px] tracking-wider text-gray-400 font-bold mb-2">
                        {lang === 'AR' ? 'التصنيفات الفرعية' : 'Subcategories'}
                      </p>
                      <div className="flex flex-col gap-1 max-h-60 overflow-y-auto cutom-scroll">
                        {mainGroup?.subGroups?.map((sub) => (
                          ...
                          >
                            ...
                            <span
                              className={`shrink-0 text-[11px] font-semibold px-1.5 py-0.5 rounded-full ${selectedSubgroup?.subGroupID === sub.subGroupID
                                ? "bg-primary/20 text-primary"
                                : "bg-gray-100 text-gray-400"
                                }`}
                            >
      ...
      <div ref={menuContentRef} className="lg:w-3/4 w-full lg:px-2 flex flex-col gap-6 scroll-mt-24">
        ...
        {user && customerMenus?.[0]?.customerMenuHeaders?.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => setSelectedMenuHeaderId(null)}
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-all border ${selectedMenuHeaderId === null
            ...
            {customerMenus[0].customerMenuHeaders.map((header, i) => (
              <button
                key={header.customerMenuHeaderId}
                onClick={() => setSelectedMenuHeaderId(header.customerMenuHeaderId)}
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-all border ${selectedMenuHeaderId === header.customerMenuHeaderId
                  ? 'bg-primary text-white border-primary shadow-sm'
                  : 'bg-white text-secondary border-gray-200 hover:border-primary hover:text-primary'
                  }`}
              >
        ...
        {!debouncedSearch.trim() && selectedSubgroup && (
