...
import { AddsPoupMobile } from "../../../components/mobile/MobileMenu";

function MobileCart() {
    const {navBarHeight}=useScreenViewStore();
    const navigate = useNavigate();
    const { lang } = useLangStore();
    const{cart,updateQuantity,getTotalPrice,serviceFee,deliveryFee,setCart,removeFromCart }=useCartStore();
    const user = useAuthStore((state) => state.user);
    const [cheackOutPopUpOpen, setCheackOutPopUpOpen] = useState(false);
        const [selectedAddsItem, setselectedAddsItem] = useState(null);
            ...
            <div className={`bg-[#f5f5f5] p-2.5 flex h-[calc(100vh-${navBarHeight}px)] items-center justify-center shadow-md`}>
                          <div className="text-center flex flex-col items-center justify-center gap-4">
                            ...
                            <button
                                          ...
                                          className="bg-primary text-white py-2 px-4 rounded hover:text-primary hover:bg-transparent border border-primary transition duration-300 ease-in-out"
                                        >
                                          {langText.continueShopping[lang]}
                                        </button>
                          </div>
                        </div>
        )}
    ...
    <div className="p-6 pb-24">
      ...
      <div className="flex flex-col w-full">
                        {cart.map((item)=>{return (
                          ...
                          selectedAddsItem={selectedAddsItem}
        ...
        <div className="mt-9">
            ...
            <div className="w-full flex flex-col gap-4">
                        <div className="flex items-center justify-between py-2 border-0 border-b border-b-light-gray">
                            ...
                            <p>{lang=="EN"?getTotalPrice():toArabicNumbers(getTotalPrice())} </p>
                        </div>
                        <div className="flex items-center justify-between py-2 border-0 border-b border-b-light-gray">
                            <p className='font-semibold'>{langText.deliveryFee[lang]}</p>
                            <p>{lang=="EN"?deliveryFee:toArabicNumbers(deliveryFee)}</p>
                        </div>
                        <div className="flex items-center justify-between py-2 border-0 border-b border-b-light-gray">
                            <p className='font-semibold'>{langText.servviceFee[lang]}</p>
                            <p>{lang=="EN"?serviceFee:toArabicNumbers(serviceFee)} </p>
                        </div>
                        <div className="flex items-center justify-between py-2 border-0 ">
                            <p className='font-semibold '>{langText.totalAmountEGP[lang]}</p>
                            <p className="text-xl text-primary">{lang=="EN"?(getTotalPrice()+serviceFee+deliveryFee):toArabicNumbers(getTotalPrice()+serviceFee+deliveryFee)}</p>
                        </div>
                    </div>
        </div>
      </div>
      ...
      <div className="fixed w-full left-0 border-t border-t-light-gray bottom-0 py-3 flex items-center justify-between gap-2 bg-white shadow">
                          <button onClick={()=>navigate(-1)} className='py-3 cursor-pointer mx-auto mt-5 border-primary border text-primary rounded-full w-2/5'>{langText.goBack[lang]}</button>
                          <button onClick={()=>{getTotalPrice()<80 ? onlineOrderToast.error(langText.yourOrderMustBeAtLeast80EGP[lang]):setCheackOutPopUpOpen(true)}} className='py-3 cursor-pointer mx-auto mt-5 bg-primary text-white rounded-full w-2/5'>{langText.pay[lang]}</button>

      </div>
      {cheackOutPopUpOpen && <CheackOutPopUp onClose={()=>setCheackOutPopUpOpen(false)}/>}
        {selectedAddsItem && (
                                          <AddsPoupMobile item={selectedAddsItem} setselectedAddsItem={setselectedAddsItem} edit={true} editedItem={editedItem}/>
